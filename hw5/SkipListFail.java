public class SkipListFail<E> {
   // Stephanie grabbed this field and the random function from DSutil.java
   // by Cliff Shaffer.
   /** Initialize the random variable */
   static private FakeRandHeight value = new FakeRandHeight();
   ...
   int randomLevel() {
       //return 1;
    //  int lev;
    //  for (lev=0; value.get() == 0; lev++); // Do nothing
    //  return lev;
      return value.get();
   }
   ...
   public void insert(int k, E newValue) {
     int newLevel = value.get();  // New node’s level
